...
### 4.1.1 Tunned Linear SVM (Tuning)

```{r}
# Tune parameters first
set.seed(1)
tune_linear <- tune(svm, diabetes_status ~ bmi + exercise_minutes + vegetable_servings + sleep_hours + age + sex, data = train_scaled, kernel = "linear", ranges = list(cost = c(0.01, 0.05, 0.1, 1, 5, 10)))
tic("Linear Tuned SVM")
...
### 4.3.1 Tunned Polynomial SVM (Tuning)

```{r}
# Tune parameters first
set.seed(1)
tune_poly <- tune(svm, diabetes_status ~ bmi + exercise_minutes + vegetable_servings + sleep_hours + age + sex, data = train_scaled, kernel = "polynomial", ranges = list(cost = c(0.1, 1, 10), degree = c(2, 3, 4), coef0 = c(0, 1, 2)))
tic("Polynomial SVM")
